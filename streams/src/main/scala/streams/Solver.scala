...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = {
    b.legalNeighbors map { case (bloc, listMove) => (bloc, listMove :: history) } toStream }
  /**
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
    ...
    neighbors filterNot { case (bloc, moves) => explored contains bloc }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case (bloc, moves) #:: xs => {
      val more = newNeighborsOnly(neighborsWithHistory(bloc, moves), explored + bloc)

      more ++ from(xs ++ more, explored + bloc)
    }
  }
  ...
  lazy val solution: List[Move] = pathsToGoal match {
    ...
    case (bloc, moves) #:: xs => moves.reverse
  }
}
